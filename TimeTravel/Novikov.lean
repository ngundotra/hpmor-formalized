...
We formalize a mathematical model of Novikov's self-consistency principle from
theoretical physics. The principle, proposed by Igor Novikov, states that if
time travel to the past is possible, any events in the past that are influenced
by a time traveler must already be part of the self-consistent history that
led to the time travel in the first place. In other words: paradoxes are
impossible because only self-consistent loops can occur.
...
PROVIDED SOLUTION
Use exists_iterate_eq_of_finite to get i < j with f^[i] s = f^[j] s. Then by periodic_point_of_iterate_eq, f^[j-i] (f^[i] s) = f^[i] s. So f^[i] s is periodic with period j-i > 0 (since i < j). Unfold periodicConsistent and use these facts.
-/
theorem novikov_periodic_consistency
    (S : Type*) [Fintype S] [Nonempty S] [DecidableEq S]
    (tl : TimeLoop S) :
    ∃ s : S, ∃ k : ℕ, tl.periodicConsistent s k := by
  -- By the pigeonhole principle, since S is finite, there exist indices i and j with i < j such that tl^[i] s = tl^[j] s.
  have h_pigeonhole : ∀ (s : S), ∃ i j : ℕ, i < j ∧ tl.transition^[i] s = tl.transition^[j] s := by
    -- By the pigeonhole principle, since S is finite, the sequence tl^[i] s must eventually repeat.
    intros s
...
PROVIDED SOLUTION
By pigeonhole: consider the Fintype.card S + 1 values f^[0] s, f^[1] s, ..., f^[Fintype.card S] s. By pigeonhole principle, two of them must be equal. So there exist i < j ≤ Fintype.card S with f^[i] s = f^[j] s.
-/
...
PROVIDED SOLUTION
From f^[i] s = f^[j] s with i < j, we need f^[j-i] (f^[i] s) = f^[i] s. Note that f^[j-i] (f^[i] s) = f^[j-i+i] s = f^[j] s = f^[i] s. Use Function.iterate_add_apply.
-/
...
PROVIDED SOLUTION
Since S is nonempty, pick any s₀ : S. Let s = tl.transition s₀. Then tl.transition s = tl.transition (tl.transition s₀) = (tl.transition ∘ tl.transition) s₀ = tl.transition s₀ = s by h. So s is a fixed point.
-/
...
PROVIDED SOLUTION
We need tl.transition c = c. Since hc says ∀ s, tl.transition^[n] s = c, apply this with s = tl.transition c to get tl.transition^[n] (tl.transition c) = c. But tl.transition^[n] (tl.transition c) = tl.transition (tl.transition^[n] c) by Function.Commute.iterate_self or by rewriting iterate. And tl.transition^[n] c = c by hc. So tl.transition c = c.
-/
theorem novikov_fixed_point_of_constant_composition
  ...
  -- By definition of `tl.selfConsistent`, we need to show that `tl.transition c = c`.
...
PROVIDED SOLUTION
If dag.edge i i held, then by order_consistent we'd have i.val < i.val, contradicting lt_irrefl.
-/
...
PROVIDED SOLUTION
Same as dag_no_self_loop: for any i, if dag.edge i i, then order_consistent gives i.val < i.val, contradiction. Can use dag_no_self_loop directly.
-/
...
PROVIDED SOLUTION
If dag.edge i j and dag.edge j i, then order_consistent gives i.val < j.val and j.val < i.val, which is a contradiction by omega.
-/
...
PROVIDED SOLUTION
The DAG is the linear chain where edge i j ↔ j.val = i.val + 1. Define state : Fin n → S by state i = f^[i.val] s₀ for an arbitrary s₀ from Nonempty S. Define evolve = f. Then for any edge i j (meaning j.val = i.val + 1), state j = f^[j.val] s₀ = f^[i.val + 1] s₀ = f (f^[i.val] s₀) = evolve (state i). Use ⟨⟨fun i => f^[i.val] (Classical.arbitrary S), f, ...⟩, rfl⟩.
-/
...
PROVIDED SOLUTION
seq.fullLoop is just a function S → S. Since S is finite and nonempty, apply novikov_periodic_consistency to the TimeLoop ⟨seq.fullLoop⟩. This gives ∃ s k, k > 0 ∧ seq.fullLoop^[k] s = s. Unfold periodicConsistent to extract the result.
-/
...
PROVIDED SOLUTION
Take s = true, k = 2. Then (!(!true)) = true. Use decide or just exact ⟨true, 2, by omega, rfl⟩.
-/
